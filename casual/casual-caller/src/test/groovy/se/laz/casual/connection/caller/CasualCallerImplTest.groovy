...
class CasualCallerImplTest extends Specification
{
   ...
   def 'tpconnect fail, TPENOENT - no such service handled'()
   {
      given:
      def serviceName = 'does not exist'
      def connectionFactory = Mock(CasualConnectionFactory)
      ...
      when:
      TpConnectReturn result = instance.tpconnect(serviceName, Mock(CasualBuffer), Flag.of(AtmiFlags.TPRECVONLY))
      then:
      result.errorState == ErrorState.TPENOENT
   }
    ...
    def 'tpconnect cache has service, but its connection factory is currently invalid'()
    {
        given:
        def serviceName = 'someservice'
        def connectionFactoryEntry = Mock(ConnectionFactoryEntry)
        ...
        when:
        def reply = instance.tpconnect(serviceName, Mock(CasualBuffer), Flag.of(AtmiFlags.TPRECVONLY))
        then:
        reply.getErrorState() == ErrorState.TPENOENT
    }
   ...
   def 'TPNOTRAN tpconnect, in transaction'()
   {
      given:
      1 * transactionManager.suspend() >> {
         Mock(Transaction)
      ...
      caller.tpCaller = Mock(TpCallerFailover)
      1 * transactionManager.resume(_)
      def flags = Flag.of(AtmiFlags.TPNOTRAN).setFlag(AtmiFlags.TPRECVONLY)
      when:
      caller.tpconnect("foo", Mock(CasualBuffer), flags)
      then:
      noExceptionThrown()
   }

   def 'TPNOTRAN tpconnect, no transaction'()
   {
      given:
      1 * transactionManager.suspend() >> {
         null
